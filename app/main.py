"""
CompProp Data — FastAPI Backend
Domain: real estate
AI Focus: property valuation, market analysis, investment scoring
Features: Market Intelligence, AI Valuations, Investment Score, Comp Analysis, Deal Pipeline
"""
...
async def lifespan(app: FastAPI):
    print(f"[CompProp Data] Starting up...")
    yield
    print(f"[CompProp Data] Shutting down...")

# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="CompProp Data API",
    description="Commercial property intelligence platform",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def root():
    return {
        "service": "CompProp Data",
        "tagline": "Commercial property intelligence platform",
        "domain": "real estate",
        "version": "1.0.0",
        "docs": "/docs",
        "status": "live",
...
Focus area: property valuation, market analysis, investment scoring.
Tagline: Commercial property intelligence platform.
Provide accurate, helpful, domain-specific responses."""
    ...
    if not openai_key:
        ...
        content = f"[DEV MODE] CompProp Data AI received: {request.prompt[:100]}"
...
async def run_agent(task: AgentTaskRequest, req: Request):
    ...
    AI_REQUESTS.labels("gpt-4o", "real estate").inc()
    return {
...
async def websocket_ai(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_text(f"[CompProp Data AI] Processing: {data[:50]}")
            await asyncio.sleep(0.1)
            await websocket.send_text(f"[STREAM] Domain: real estate | Response ready")
