// Push Notifications — CompProp Data
...
export async function registerForPush(): Promise<string|null> {
  ...
  if (Platform.OS === 'android') {
    await N.setNotificationChannelAsync('default', {
      name: 'CompProp Data', importance: N.AndroidImportance.MAX,
      vibrationPattern: [0,250,250,250], lightColor: '#10B981',
    });
  }
...
export async function syncToken(token: string): Promise<void> {
  try {
    await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/users/push-token`, {
      ...
      body: JSON.stringify({ token, type: 'expo', app: 'CompProp Data' }),
    });
...
export async function notifyAIResult(headline: string, summary: string): Promise<string> {
  return scheduleNotification(headline, summary, { type: 'ai', app: 'CompProp Data' });
}
